...
<!--        <el-dialog title="添加查询条件" :visible.sync="filterVisible" :close-on-click-modal="false">
        ...
        <el-table :data="buyOrders" ref="buyOrders" v-loading="listLoading" :span-method="arraySpanMethod"
            ...
            <el-table-column label="库存操作明细" align="center" width="800">
                <el-table-column label="名称" prop="storeOperationItems_name" min-width="120"  align="center"></el-table-column>
                <el-table-column label="种类" prop="storeOperationItems_sort" min-width="120"  align="center"></el-table-column>
                <el-table-column label="规格" prop="storeOperationItems_spec" min-width="120"  align="center"></el-table-column>
                <el-table-column label="单位" prop="storeOperationItems_unit" min-width="120"  align="center"></el-table-column>
                <el-table-column label="数量" prop="storeOperationItems_number" min-width="120"  align="center"></el-table-column>
                <el-table-column label="实际单价" prop="storeOperationItems_price" min-width="120"  align="center"></el-table-column>
            </el-table-column>

            <el-table-column label="库存操作信息"  align="center" width="1060">
                ...
                <el-table-column label="操作类型" prop="type" min-width="200" :formatter="formatTypeFlag" sortable="custom" align="center"></el-table-column>
                ...
                <el-table-column label="审批意见" prop="opinion" min-width="200" sortable="custom" align="center"></el-table-column>
            </el-table-column>
        ...
        <el-dialog fullscreen title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false" @close="clearValidate('editForm')">
            <el-form :model="editObj" label-width="80px" :rules="editFormRules" ref="editForm">
                ...
                <el-form-item label="操作类型" prop="type">
                    <el-select v-model="editObj.type" placeholder="请选择" @change="selectEditDepartment">
                        <el-option
                ...
                <el-form-item label="物品明细" prop="storeOperationItems">

                    <el-table :data="editObj.storeOperationItems"
                              show-summary
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                                <el-button type="danger" size="small" icon="el-icon-minus" @click="handleRemoveBuyOrderItem(scope.$index, editObj.storeOperationItems)">
                                </el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                    ...
                    <el-form-item style="margin-top: 10px;">
                        <el-button type="primary" size="medium" icon="el-icon-plus" @click="handleAddBuyOrderItem(editObj.storeOperationItems)"></el-button>
                    </el-form-item>
        ...
        <el-dialog title="新增" fullscreen :visible.sync="addFormVisible" :close-on-click-modal="false" @close="clearValidate('addForm')">
            <el-form :model="addObj" label-width="80px" :rules="addFormRules" ref="addForm">
                ...
                <el-form-item label="操作类型" prop="type">
                    <el-select v-model="addObj.type" placeholder="请选择" @change="selectEditDepartment">
                        <el-option
                                v-for="item in allTypes"
                                :key="item.value"
                                :label="item.name"
                ...
                <el-form-item label="物品明细" prop="storeOperationItems">
                    ...
                    <el-table :data="addObj.storeOperationItems"
                              show-summary
                              :summary-method="getSummaries"
                              border
                              height="350px" max-height="999999px"
                        ...
                        <el-table-column
                                ...
                                width="50"  align="center">
                                <template slot-scope="scope">
                                    <el-button type="danger" size="small" icon="el-icon-minus" @click="handleRemoveBuyOrderItem(scope.$index, addObj.storeOperationItems)">
                                    </el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                    ...
                    <el-form-item style="margin-top: 10px;">
                        <el-button type="primary" size="medium" icon="el-icon-plus" @click="handleAddBuyOrderItem(addObj.storeOperationItems)"></el-button>
                    </el-form-item>
    ...
    export default {
        data() {
            ...
            var checkBuyOrderItems = (rule, value, callback) => {
                var buyOrderItems = this.editObj.storeOperationItems;
        ...
        methods: {
            ...
            checkAddBuyOrderItems() {
                var buyOrderItems = this.addObj.storeOperationItems;
                let  rexPrice = /^[0-9]+(.[0-9]{1,2})?$/;
                //let  rexNum = /^[1-9]\d*$/;
                let  rexNum =  /^\d+$/;
                this.canAdd = true;
            ...
            getBuyOrders() {
                ...
                requestApi.storeOperation.findByParam(this, formData).then(res => {
                    this.listLoading = false;
                    res = res.body;
                    if(res.code === Enum.SUCCESS.code){
                        this.pagination.total = res.data.total;
                        let rs = getNewDataAndMap("storeOperationItems",res.data.data);
                    ...
                    if (valid) {
                        ...
                        if(this.canAdd) {
                            this.$confirm('确认提交吗？', '提示', {}).then(() => {
                                ...
                                requestApi.storeOperation.add(this, para).then(res => {
                                    res = res.body;
            ...
            quickInput() {
                ...
                this.$confirm('确认入库吗？', '提示', {
                    ...
                    this.listLoading = true;
                    requestApi.storeOperation.quickInput(this, para).then(res => {
                        this.listLoading = false;
                        res = res.body;
                        if(res.code === Enum.SUCCESS.code){
            ...
            approve(id, approvalResult) {
                ...
                this.$confirm('确认提交吗？', '提示', {}).then(() => {
                    this.listLoading = true;
                    requestApi.storeOperation.approve(this, para).then(res => {
                        this.listLoading = false;
                        res = res.body;
                        if (res.code === Enum.SUCCESS.code) {
                            this.getBuyOrders();
            ...
            batchApprove(approvalResult){
                ...
                this.$confirm(model_msg, '提示', {
                    ...
                    this.listLoading = true;
                    requestApi.storeOperation.approve(this, approvalArr).then(res => {
                        this.listLoading = false;
                        res = res.body;
                        if(res.code === Enum.SUCCESS.code) {
                            this.getBuyOrders();
            ...
            batchRemove() {
                ...
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    this.listLoading = true;
                    requestApi.storeOperation.deletes(this, para).then(res => {
                        this.listLoading = false;
                        res = res.body;
                        if(res.code === Enum.SUCCESS.code) {
                            this.getBuyOrders();
            ...
            handleEdit(index,row) {
                ...
                requestApi.storeOperation.findById(this, formData).then(res => {
                    this.listLoading = false;
                    res = res.body;
                    this.editObj = res;
                }, error=>{
                    ...
                    if (valid) {
                        ...
                        if(this.canAdd) {
                            this.$confirm('确认提交吗？', '提示', {}).then(() => {
                                ...
                                requestApi.storeOperation.update(this, para).then(res => {
                                    res = res.body;
            ...
            handleDel(index, row) {
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    this.listLoading = true;
                    let para = { id: row.id };
                    requestApi.storeOperation.delete(this, para).then(res => {
                            this.listLoading = false;
                            res = res.body;
                            if(res.code === Enum.SUCCESS.code) {
                                this.getBuyOrders();
