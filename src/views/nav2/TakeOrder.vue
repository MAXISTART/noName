<template>
    <div>
        ...
        <el-col :span="24" class="toolbar">
            ...
            <el-button type="primary" size="midium" :disabled="this.sels.length === 0" @click="quickOutput">快速出库</el-button>
        </el-col>
...
<!--        <el-dialog title="添加查询条件" :visible.sync="filterVisible" :close-on-click-modal="false">
        ...
        <el-dialog fullscreen title="编辑" :visible.sync="editFormVisible" :close-on-click-modal="false" @close="clearValidate('editForm')">
            <el-form :model="editObj" label-width="80px" :rules="editFormRules" ref="editForm">
                ...
                <el-form-item label="物品明细" prop="takeOrderItems">

                    <el-table :data="editObj.takeOrderItems"
                              show-summary
                        ...
                        <el-table-column label="归还数量" prop="returnNumber" align="center">
                            <template slot-scope="scope">
                                <el-input v-model="scope.row.returnNumber" placeholder="请输入归还数量"></el-input>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                                <el-button type="danger" size="small" icon="el-icon-minus" @click="handleRemoveTakeOrderItem(scope.$index, editObj.takeOrderItems)">
                                </el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                    ...
                    <el-form-item style="margin-top: 10px;">
                        <el-button type="primary" size="medium" icon="el-icon-plus" @click="handleAddTakeOrderItem(editObj.takeOrderItems)"></el-button>
                    </el-form-item>
        ...
        <el-dialog title="新增" fullscreen :visible.sync="addFormVisible" :close-on-click-modal="false" @close="clearValidate('addForm')">
            <el-form :model="addObj" label-width="80px" :rules="addFormRules" ref="addForm">
                ...
                <el-form-item label="物品明细" prop="takeOrderItems">
                    ...
                    <el-table :data="addObj.takeOrderItems"
                              show-summary
                              :summary-method="getSummaries"
                              border
                              height="350px" max-height="999999px"
                        ...
                        <el-table-column
                                ...
                                width="50"  align="center">
                                <template slot-scope="scope">
                                    <el-button type="danger" size="small" icon="el-icon-minus" @click="handleRemoveTakeOrderItem(scope.$index, addObj.takeOrderItems)">
                                    </el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                    ...
                    <el-form-item style="margin-top: 10px;">
                        <el-button type="primary" size="medium" icon="el-icon-plus" @click="handleAddTakeOrderItem(addObj.takeOrderItems)"></el-button>
                    </el-form-item>
...
<script>
    ...
    export default {
        data() {
            ...
            // 编辑物品的验证
            var checkTakeOrderItems = (rule, value, callback) => {
                var takeOrderItems = this.editObj.takeOrderItems;
            ...
            return {
                ...
                editFormRules: {
                    ...
                    takeOrderItems: [
                        ...
                        { validator: checkTakeOrderItems, trigger: 'blur' },
                    ],
                    price: [
        ...
        methods: {
            ...
            checkAddTakeOrderItems() {
                var takeOrderItems = this.addObj.takeOrderItems;
                let  rexPrice = /^[0-9]+(.[0-9]{1,2})?$/;
                //let  rexNum = /^[1-9]\d*$/;
                let  rexNum =  /^\d+$/;
                this.canAdd = true;
            ...
            handleSelect(takeOrderItem, selected_item) {
                takeOrderItem.goodId = selected_item.id;
                takeOrderItem.name = selected_item.name;
                takeOrderItem.sort = selected_item.sort;
                takeOrderItem.spec = selected_item.spec;
                takeOrderItem.unit = selected_item.unit;
                takeOrderItem.price = selected_item.price;
                takeOrderItem.returnNumber = 0;
            },
            // 远程搜索用户名称
            ...
            getTakeOrders() {
                ...
                requestApi.takeOrder.findByParam(this, formData).then(res => {
                    ...
                    if(res.code === Enum.SUCCESS.code){
                        ...
                        let rs = getNewDataAndMap("takeOrderItems",res.data.data);
                        this.takeOrders = rs["data"];
                        this.dataMap = rs["map"];
                    }
                }, err => {
                    msgUtils.error(this, Enum.SYSTEM_ERROR.msg);
            ...
            handleRemoveTakeOrderItem(index, takeOrderItems) {
                takeOrderItems.splice(index, 1);
            },

            // 点击添加框或者编辑框的添加物品明细按钮
            handleAddTakeOrderItem(takeOrderItems) {
                let obj = {
                    name: '',
                    sort: '',
                    spec: '',
            ...
            quickOutput() {
                ...
                this.$confirm('确认出库吗？', '提示', {
                    ...
                    this.listLoading = true;
                    requestApi.takeOrder.quickOutput(this, para).then(res => {
                        this.listLoading = false;
                        res = res.body;
                        if(res.code === Enum.SUCCESS.code){
            ...
            approve(id, approvalResult) {
                ...
                this.$confirm('确认提交吗？', '提示', {}).then(() => {
                    this.listLoading = true;
                    requestApi.takeOrder.approve(this, para).then(res => {
                        this.listLoading = false;
                        res = res.body;
                        if (res.code === Enum.SUCCESS.code) {
                            this.getTakeOrders();
            ...
            batchApprove(approvalResult){
                ...
                this.$confirm(model_msg, '提示', {
                    ...
                    this.listLoading = true;
                    requestApi.takeOrder.approve(this, approvalArr).then(res => {
                        this.listLoading = false;
                        res = res.body;
                        if(res.code === Enum.SUCCESS.code) {
                            this.getTakeOrders();
            ...
            batchRemove() {
                ...
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    this.listLoading = true;
                    requestApi.takeOrder.deletes(this, para).then(res => {
                        this.listLoading = false;
                        res = res.body;
                        if(res.code === Enum.SUCCESS.code) {
                            this.getTakeOrders();
            ...
            handleEdit(index,row) {
                ...
                let formData = new FormData();
                formData.append('takeOrderId', row.id);
                requestApi.takeOrder.findById(this, formData).then(res => {
                    this.listLoading = false;
                    res = res.body;
                    this.editObj = res;
                }, error=>{
                    ...
                    if (valid) {
                        ...
                        if(this.canAdd) {
                            this.$confirm('确认提交吗？', '提示', {}).then(() => {
                                ...
                                requestApi.takeOrder.update(this, para).then(res => {
                                    res = res.body;
            ...
            handleDel(index, row) {
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    this.listLoading = true;
                    let para = { id: row.id };
                    requestApi.takeOrder.delete(this, para).then(res => {
                            this.listLoading = false;
                            res = res.body;
                            if(res.code === Enum.SUCCESS.code) {
                                this.getTakeOrders();
